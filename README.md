...
MATCH (a:Function)-[:CALLS*1..3]->(b:Function) WHERE a.name = 'main' RETURN b.name LIMIT 10
...
Supported result clauses include `RETURN DISTINCT`, `count(...)`, `ORDER BY`, `SKIP`, and `LIMIT`. Variable-length relationships require an explicit type and bounded range (`*1..5` max), a selective `WHERE` anchor on either endpoint, and no relationship alias because a path can contain multiple edges.
