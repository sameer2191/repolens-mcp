...
- **Route and protocol call edges**: stores literal HTTP requests as `http_call` nodes, connects callers with `CALLS_HTTP_ENDPOINT`, links matching in-repo routes with `HTTP_CALLS`, and resolves GraphQL, gRPC, and tRPC call sites to declared protocol targets.
...
- **Protocol surfaces**: extracts GraphQL operations/types, gRPC services/RPC routes from protobuf, OpenAPI routes, common tRPC procedures/calls, and typed call edges such as `CALLS_GRAPHQL_OPERATION`, `CALLS_GRPC_METHOD`, and `CALLS_TRPC_PROCEDURE`.
...
- Trace modes: `trace_path` can focus on call paths, value propagation through `DATA_FLOWS`, cross-service HTTP/event/protocol paths, or all nearby edges.
...
- GraphQL, gRPC, and OpenAPI: `.graphql`, `.gql`, `.proto`, OpenAPI JSON, and OpenAPI YAML files produce protocol nodes; protobuf `rpc` methods become route nodes using `/Service/Method` paths, OpenAPI `{id}` path params normalize to `:id`, and matching call sites link to protocol targets.
- tRPC: common procedure declarations and client calls become `trpc_procedure` and `trpc_call` nodes, with resolved `CALLS_TRPC_PROCEDURE` edges when names match.
