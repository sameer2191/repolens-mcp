...
- CI runs type-check, tests, package dry-run, CycloneDX SBOM generation, self-indexing, and architecture output; separate workflows cover OpenSSF Scorecard and release build-provenance attestations.
...
- A no-op incremental run on the large validation repo completed in 275 ms while preserving a 5,812-symbol, 38,634-edge graph with 153 Next.js route nodes, 642 resolved import edges, 5,957 type-use edges, 976 data-flow edges, and 387 locked dependencies.
