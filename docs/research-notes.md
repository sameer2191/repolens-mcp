...
- CI runs type-check, tests, production dependency audit, package dry-run, package contents gating, CycloneDX SBOM generation, self-indexing, and architecture output; separate workflows cover OpenSSF Scorecard and release build-provenance attestations.
- `llms.txt` and `docs/agent-guide.md` provide concise agent-facing operating instructions, validation commands, and local-data boundaries in the npm package.
...
- The package contents gate blocks local graph memory, SQLite databases, graph packages, tests, source TypeScript, and private validation output from the published npm artifact.
