...
- Executable `agent-hook` / `hook-augment` support turns broad-search hook payloads into non-blocking RepoLens context reminders while skipping direct Read/Edit/Write tools; `--with-query` can opt in to local graph metadata matches when the maintainer wants DB-backed augmentation.
