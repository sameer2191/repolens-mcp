...
- Node test suite passed: 72 tests, 71 passing, 0 failures, 1 sandbox-only dashboard socket skip.
...
- Packed artifact: `repolens-mcp-1.0.0.tgz`, 199,488 bytes packed, 1,017,746 bytes unpacked, 92 runtime/doc entries.
...
- `hook-augment --claude` smoke test emitted Claude-compatible `hookSpecificOutput.additionalContext` for a fake Grep PreToolUse payload without querying the local graph by default; the opt-in `--with-query` smoke on the fixture graph appended local symbol metadata matches, and a fake Read payload exited 0 with no blocking output.
