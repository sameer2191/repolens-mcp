...
- Node test suite passed: 32 tests, 0 failures.
...
- Packed artifact: `repolens-mcp-1.0.0.tgz`, 142,301 bytes packed, 740,303 bytes unpacked, 71 runtime entries.
...
- Symbols: 782
- Edges: 2,833
- Lines indexed: 14,617
- Full index elapsed: 989 ms
- No-op incremental elapsed: 65 ms
- No-op incremental unchanged files: 67
- Full-text code-search rows: 13,300 `code_lines` rows and 13,300 `code_fts` rows
- Local vector rows: 583 `symbol_vectors` rows at 384 dimensions; `vector "local vector search"` returned `LocalVector`, `vectorSearch`, and `VectorSearchMatch` as the top three results.
...
- MCP server tools registered: 33
...
- Redacted secret scan: 0 high/medium-confidence findings across 11,857 indexed non-test lines.
...
- Type relationship rows: 375 `USES_TYPE` edges, 4 `INHERITS` edges, and 1 `IMPLEMENTS` edge.
...
- Project catalog status: `list-projects` and `project-status repolens-mcp` returned the self graph with live totals of 67 files, 782 symbols, and 2,833 edges.
...
- Imported package totals: 67 files, 782 symbols, 2,833 edges, plus 583 persisted vector rows at 384 dimensions.
